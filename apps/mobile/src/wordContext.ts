...
// rel_syn = synonyms, rel_bgb = frequent predecessors, rel_bga = followers;
// phrasal verbs are followers that happen to be particles.
...
const CACHE_KEY = "wordup-context";
let cache: Record<string, WordCtx> = {};
let loaded = false;
...
const STOP = new Set(["the", "a", "an", "of", "to", "and", "or", "is", "am",
  "was", "be", "been", "being", "it", "that", "this", "these", "those", "his",
  "her", "its", "my", "your", "our", "as", "at", "by", "he", "she", "they",
  "we", "i", "you", "not", "are", "were", "have", "has", "had", "but", "with",
  "from", "in", "on", "so", "if", "than", "then", "there", "who", "which",
  "what", "when", "will", "would", "can", "could", "do", "does", "did", "for",
  "any", "some", "no", "one", "two", "all", "more", "most", "other", "such",
  "own", "very", "how", "why", "where", "also", "just", "about"]);
...
export async function fetchWordContext(word: string, pos?: string): Promise<WordCtx> {
  ...
  if (misses.has(w)) return EMPTY;

  const get = (url: string) => fetch(url).then(r => (r.ok ? r.json() : [])).catch(() => null);
  ...
  const phrasal = (pos || "").includes("verb")
    ...
    : [];
  const colloc = [
    ...clean(before).filter(x => !STOP.has(x)).slice(0, 5).map(x => `${x} ${w}`),
    ...clean(after).filter(x => !STOP.has(x) && !PARTICLES.has(x)).slice(0, 5).map(x => `${w} ${x}`),
  ];
  const synonyms = clean(syn).slice(0, 10);

  cache[w] = { phrasal, colloc, synonyms };
